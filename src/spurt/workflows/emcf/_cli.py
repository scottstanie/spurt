...
def dem_error_main(args=None):
    """Entry point for DEM error estimation without full unwrapping.

    Runs only the link model estimation (velocity and DEM error) on spatial
    gradients, skipping the temporal and spatial MCF unwrapping. Produces
    GeoTIFFs of estimated DEM error, velocity, and model coherence.

    This is useful as a first pass: estimate DEM error, re-geocode the
    data with corrected heights, then run the full EMCF workflow.
    """
    parser = argparse.ArgumentParser(
        prog="spurt-dem-error",
        description="Estimate DEM error and velocity without full phase unwrapping.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--version", action="version", version=spurt.__version__)
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "--outputdir",
        default="./dem_error",
        help="Output folder for DEM error rasters.",
    )
    parser.add_argument(
        "--tempdir",
        default="./dem_error_tmp",
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Number of workers for model estimation. <=0 uses ncpus - 1.",
    )
    parser.add_argument(
        "-b",
        "--batchsize",
        type=int,
        default=150000,
        help="Links per batch for model estimation.",
    )
    parser.add_argument(
        "-c",
        "--coh",
    ...
    # Link model arguments (required for this workflow)
    parser.add_argument(
        "--baseline-csv",
        type=str,
        required=True,
        help="Path to CSV with perpendicular baselines.",
    )
    parser.add_argument(
        "--wavelength",
        type=float,
    ...
    # Estimate DEM error (no MCF unwrapping)
    estimate_dem_error_tiles(
        stack, g_time, gen_settings, solv_settings, link_model_settings
    )
    ...
    logger.info("Completed DEM error estimation workflow.")
